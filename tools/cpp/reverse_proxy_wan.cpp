...
int cmd_listen_socket = 0;          // 命令通道监听的socket
int cmd_connect_socket = 0;         // 命令通道连接的socket
...
int main(int argc, char *argv[])
{
    ...
    pactive.add_p_info(30, "reverse_proxy_wan");
    ...
    if(cmd_listen_socket < 0)
    {
        log_file.write("init cmd_listen_socket(%s) failed.\n", argv[3]);
        EXIT(-1);
    }
    
    // 等待内网程序的连接请求（cmd_listen_socket为阻塞模式且并未交给epoll）
    struct sockaddr_in client_addr;
    socklen_t client_addr_len = sizeof(client_addr);
    cmd_connect_socket = accept(cmd_listen_socket, (struct sockaddr*)&client_addr, &client_addr_len);
    if(cmd_connect_socket < 0)
    {
        log_file.write("accept cmd_connect_socket(%s) failed.\n", argv[3]);
        EXIT(-1);
    }
        ...
        for(int i = 0; i < in_fds; ++i)
        {
            ...
            for(j = 0; j < v_route.size(); ++j)
            {
                ...
                if(evs[i].data.fd == v_route[j].listen_socket)
                {
                    ...
                    if(dst_socket < 0)
                    {
                        log_file.write("connect connect local program error: %s\n", strerror(errno));
                        close(src_socket);
                        break;
                    }
                    if(dst_socket >= MAX_SOCKET_NUM)
                    {
                        log_file.write("when connect local program socket, too many sockets(%d > %d)\n", dst_socket, MAX_SOCKET_NUM);
                        close(src_socket);
                        close(dst_socket);
                        break;
                    }
...
{
    printf("\n");
    printf("Using: ./reverse_proxy_wan log_file init_file cmd_port\n");
    printf("Example: ./reverse_proxy_wan /root/C++/selfC++Framework/log/reverse_proxy_wan.log ./reverse_proxy_wan.conf 5001\n");
    printf("\t/root/C++/selfC++Framework/tools/cpp/procctl 4 ./reverse_proxy_wan /root/C++/selfC++Framework/log/reverse_proxy_wan.log ./reverse_proxy_wan.conf 5001\n");
    printf("parameters: \n");
    printf("\tlog_file   本程序运行的日志文件。\n");
    printf("\tinit_file  本程序的初始化文件。\n");
    printf("\tcmd_port   本程序与内网代理程序的通讯端口。\n");
    printf("brief: \n");
    printf("\t本程序为tcp反向代理-外网端示例程序\n");
}
