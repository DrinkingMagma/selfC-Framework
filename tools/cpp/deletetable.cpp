/**
 * @file deletetable.cpp
 * @brief 清除表中的数据（数据清理）
 * @details 该程序用于清理数据库表中的数据，支持按条件删除，并通过批量处理提高效率
 */
...
struct ST_Arg
{
    ...
    char table_name[31];    ///< 待清理的表名
    char key_col[31];       ///< 待清理的表的唯一键字段名，建议用rowid，效率最高
    char where[1001];       ///< 待清理的数据需要满足的条件，即SQL语句中的where部分
    int  max_count;         ///< 执行一次SQL删除的记录数，建议在100-500之间
...
int main(int argc, char *argv[])
{
    ...
    // 开启自动提交事务，连接数据库
    if(conn.connecttodb(st_arg.conn_str, "Simplified Chinese_China.AL32UTF8", true) != 0)
    {
...
{
    print_dash_line(60);
    printf("Usage: /root/C++/selfC++Framework/tools/bin/deletetable log_file_name config_xml_file_name\n");
    printf("Example 1: /root/C++/selfC++Framework/tools/bin/deletetable /root/C++/selfC++Framework/log/deletetable.log /root/C++/selfC++Framework/tools/cpp/deletetable_config.xml\n");
    printf("Example 2: /root/C++/selfC++Framework/tools/bin/procctl 3600 /root/C++/selfC++Framework/tools/bin/deletetable /root/C++/selfC++Framework/log/deletetable.log /root/C++/selfC++Framework/tools/cpp/deletetable_config.xml\n");
    printf("brief: 清除指定表中的数据\n");
    printf("Param: \n");
    printf("    log_file_name 日志文件\n");
    printf("    config_xml_file_name 配置文件，参数说明如下\n");
    printf("        conn_str        数据库的连接参数，格式：username/passwd@tnsname。\n");
    printf("        table_name      待清理数据表的表名。\n");
    printf("        key_col         待清理数据表的唯一键字段名，可以用记录编号，如keyid，建议用rowid，效率最高。\n");
    ...
    printf("        start_time      程序运行的时间区间，例如02,13表示：如果程序运行时，踏中02时和13时则运行，其它时间不运行。"
                                        "如果start_time为空，本参数将失效，只要本程序启动就会执行数据清理，"
...
bool _migratetable()
{
    ...
    while (true)
    {
        ...
        // 若数组中的记录数达到了max_count，则执行一次删除数据
    ...
    // 输出删除结果
    if(stmt_del.rpc() > 0)
        logfile.write("delete %d rows from table(%s), spend %.02f seconds.\n", stmt_del.rpc(), st_arg.table_name, timer.elapsed());
