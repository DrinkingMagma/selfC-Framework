...
int main(int argc,char *argv[])
{
    ...
    if (ftp.nlist("/home/centos/", "/tmp/list/tmp.list")==false) 
    { 
        printf("ftp.nlist() failed.\n"); return -1; 
    }
    printf("ftp.nlist() successed.\n");
    cout << "ret=" << ftp.response() << endl;

    cifile ifile;    // 采用开发框架的cifile类来操作list文件。
    string strFileName;

    ifile.open_file("/tmp/list/tmp.list");  // 打开list文件。

    while(true)    // 获取每个文件的时间和大小。
    {
        if (ifile.read_line(strFileName)==false) break;
